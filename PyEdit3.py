...
from PyQt5.QtWidgets import (QWidget, QVBoxLayout, QApplication, QFileDialog, QMessageBox,
                            QLabel, QComboBox, QAction,  QPlainTextEdit,
...
class QSC(QsciScintilla):
    ...
    def __init__(self, parent=None):
        ...
        font = QFont('Noto Mono', 10, QFont.Normal)
        #font.setFixedPitch(True)
        self.setFont(font)
        self.setMarginsFont(font)

        self.setSelectionBackgroundColor(QColor("#204a87"))
        self.setSelectionForegroundColor(QColor("#eeeeec"))
        ...
        self.setMarginsBackgroundColor(QColor("#aabbcc"))
        self.setMarginsForegroundColor(QColor("#2e3436"))

        self.setFoldMarginColors(QColor("#e2e2e2"), QColor("#e2e2e2"))
        ...
        lexer.setPaper(QColor("#e2e2e2"))

        lexer.setColor(QColor('#204a87'), QsciLexerPython.Keyword)
        #lexer.setColor(QColor('#204a87'), QsciLexerPython.Default)
        #lexer.setColor(QColor('#888a85'), QsciLexerPython.Comment)
        ...
        lexer.setColor(QColor('#8f5902'), QsciLexerPython.TripleSingleQuotedString)
        lexer.setColor(QColor('#8f5902'), QsciLexerPython.TripleDoubleQuotedString)
        ...
        lexer.setColor(QColor('#1c1c1c'), QsciLexerPython.UnclosedString)
        lexer.setColor(QColor('#204a87'), QsciLexerPython.HighlightedIdentifier)
        lexer.setColor(QColor('#75507b'), QsciLexerPython.Decorator)
        ...
        ## setup autocompletion # ~/.local/lib/python3.9/site-packages/PyQt5/Qt5/qsci/api/python/
        path = os.path.dirname(PyQt5.__file__)
        #print(path)
        self.api = QsciAPIs(lexer)
        pyqt_path = os.path.join(path, 'Qt5/qsci/api/python/QScintilla.api')
        #print(pyqt_path)
        py3_path = os.path.join(path, 'Qt5/qsci/api/python/Python-3.9.api')
        
        pyqt_webengine_path = os.path.join(path, 'Qt5/qsci/api/python/PyQtWebEngine.api')
        
        pyqt_chart_path = os.path.join(path, 'Qt5/qsci/api/python/PyQtChart.api')
        ...
        if self.api.load(pyqt_webengine_path):
            print("Qt5.WebEngine api loaded")
        else:
            print("Qt5.WebEngine api not loaded!")
            
        if self.api.load(pyqt_chart_path):
            print("QtChart api loaded")
        else:
            print("QtCharte api not loaded!")
...
class myEditor(QMainWindow):
    def __init__(self, parent = None):
        ...
        for v in versions_list:
            ...
            self.python_versions_list.append(version)

        default_version = sys.version.split(" ")[0].rpartition(".")[0]
        self.python_version = f"python{default_version}"

        print("default python3 version:", self.python_version)

        self.words = []
        self.root = QFileInfo.path(QFileInfo(QCoreApplication.arguments()[0]))
        self.checkmycode = f"{self.root}/checkmycode"
        self.bookmarkslist = []
        print("working dir is: ", self.root)
        self.logo = os.path.join(self.root, "pyedit3_32.png")
        ...
        self.windowList = []
        self.recentFileActs = []
        self.settings = QSettings("PyEdit3", "pyedit3")
        ...
        self.mainText = "#!/usr/bin/env python3\n# -*- coding: utf-8 -*-\n"
        self.fname = ""
        self.filename = ""
        ...
        tb.addSeparator()
        tb.addAction(QIcon.fromTheme("color-picker"),"change Color", self.changeColor)
        ###insert templates
        tb.addSeparator()
        ...
        self.python_version_combo.setToolTip("python Version")
        self.python_version_combo.addItems(self.python_versions_list)
        tb.addWidget(self.python_version_combo)

        tb.addSeparator()

        self.py3Act = QAction(f"run in {self.python_version} (F5)", self, shortcut="F5",
                statusTip=f"run in {self.python_version} (F5)", triggered=self.runPy3)
        ...
        self.shToggleAction.setIcon(QIcon.fromTheme("pane-hide-symbolic"))
        ...
        tbf.addAction(QAction(QIcon.fromTheme("checkbox"), "check Code",
        ...
        helpmenu.addAction(QAction(QIcon.fromTheme('devhelp'), "Dev Help", self,
                            triggered = self.showDevHelp, shortcut = "F1"))

        #helpmenu.addAction(QAction(QIcon.fromTheme('help-faq'), "GTK+ 3 Tutorial", self,
        #                    triggered = self.docs))
        ...
        self.side_list.itemClicked.connect(self.gotoBookmarkFromSideList)
    ...
    def docs(self):
        url = f"{self.root}/docs/html/index.html"
    ...
    def runInTerminal(self):
        ...
        if not self.editor.text() == self.mainText:
            if self.filename:
                self.statusBar().showMessage("running " + self.filename + " in python3")
                self.fileSave()
                self.shellWin.clear()
                dname = QFileInfo(self.filename).filePath().replace(QFileInfo(self.filename).fileName(), "")

                self.python_version = f"python{self.python_version_combo.currentText()}"
                print("using python3 version:", self.python_version)
                cmd = f'gnome-terminal -- {self.python_version} -- "{dname}{self.strippedName(self.filename)}"'
    ...
    def handleFM(self):
        if not self.filename == "":
            QProcess.startDetached("xdg-open", [os.path.dirname(self.filename)])
        else:
            QProcess.startDetached("xdg-open", [os.path.dirname(self.root)])
    ...
    def contextMenuRequested(self, point):
        ...
        cmenu.addSeparator()
        cmenu.addAction(QIcon.fromTheme("color-picker"),"insert QColor", self.insertColor)

        cmenu.addSeparator()
        cmenu.addAction(QIcon.fromTheme("color-picker"),"change Color", self.changeColor)
    ...
    def gotoBookmark(self):
        ...
        self.editor.findFirst(btext, False, True, True, True)
    ...
    def setCurrentFile(self, fileName):
        ...
        if not fileName == self.tempfile or not fileName.startswith("/tmp"):
